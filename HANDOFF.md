...
| 項目 | 状態（2026-09-23 実測） |
|---|---|
| 本番 | <https://newproduct.fun-create.co.jp> ／ `127.0.0.1:8794` ／ `a8b16a4` |
...
| DB | `data/newproduct.db`（SQLite・WAL）。migrations 001〜013 適用済み |
...
年間プランの枠・機会カレンダー）まで。**
`REQUIREMENTS.md` で `実装済` 86件・`実装中` 11件・`検証済` 2件・`未着手` 68件・`見送り` 8件（全175件）。
**第15章（自動化依頼・F-15）を 2026-09-24 に新設**（FR-159〜175）。
...
| **年間プラン2026年度（策定中）の承認** | 十文字さん | **2026-09-26 から** | FR-85。移行した26枠を画面で見て「承認」を押すと「正」になる。それまで「今月の枠の消化」は未計測のまま |
| **案件20件と枠26本の紐づけ** | 商品開発部 | **2026-09-26 から** | 名前が揺れているため自動では結びつけていない。候補は確認表。画面の「案件にする」で紐づける |
| **準備シート6本の本文（カルテの節）の移行** | 十文字さん（Drive を開く許可） | **2026-09-26 から** | F-4-8 の残り。TSV に無く、スプレッドシートを1本ずつ読む必要がある |
...
1. **本番の画面で目視する**（`実装済` 86件のうち、見たものを `検証済` へ上げる）。
