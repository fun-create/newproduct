...
| 実データ | アイデア **881件**・機会 **年間53＋ライフ31件**（採点19）・**自動化依頼15件**（うち11件が標準タスクに無い）。**案件0件・タスク0件**（業務では未使用） |
| AI予算 | 枠 `newproduct-*: 5.0`（image と text で**分け合う**）。口は `http://127.0.0.1:8789/api/ai-usage`。**モデルを呼ぶ実装はまだ無い** |
